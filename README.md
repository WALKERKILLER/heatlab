...
[![Version](https://img.shields.io/badge/version-0.3.1-green.svg)](https://github.com/WALKERKILLER/heatlab/releases/tag/v0.3.1)
...
> ```
>
> **不想安装 Python？** 直接下载 [HeatLab-Web-win64.zip（v0.3.1 正式版）](https://github.com/WALKERKILLER/heatlab/releases/download/v0.3.1/HeatLab-Web-win64.zip)，解压后双击 `HeatLab-Web.exe`；它会自动启动本地 Flask 后端并打开默认浏览器。演示版前端仍从 CDN 加载 Vue、Chart.js、Plotly 与 KaTeX，因此演示电脑需要联网。
...
| 专题 | 内容 |
|---|---|
| **热力学 / 理想气体** | 七组单变量实验：温度/压强的微观意义、等温/等容/等压过程、两室热交换、热力学第一定律；共享 `PV=nRT`、3D 分子热运动、匹配的 P-V/P-T/V-T 关系图与能量读数 |
| **布朗运动** | 显式液体分子热运动与两体弹性碰撞、轨迹与 MSD / 扩散常数；可见碰撞计数、花粉碰撞点高亮、方向箭头/速度矢量/轨迹渐隐开关 |
| **麦克斯韦速率分布** | 理论概率密度 + 蒙特卡洛直方图 + 粒子动画；**速率 f(v) 与水平速度分量 v_x 高斯分布双图对照**（线性轴、y 上限固定） |
| **伽尔顿板** | 蒙特卡洛下落路径（漏斗 → 钉板 → 狭槽 hexagonal 堆积）与二项分布对照 |

三种使用方式：

1. **Windows Web 一键演示版（推荐）** — [下载 v0.3.1 正式版](https://github.com/WALKERKILLER/heatlab/releases/tag/v0.3.1)，解压后双击 `HeatLab-Web.exe`
2. **浏览器实时版** — `heatlab-web` → <http://127.0.0.1:8765>，源码开发与本地调试入口
3. **旧版 PySide6 桌面版** — `heatlab` / `heatlab-ideal-gas` 等，仅保留用于源码对照，不再作为主要演示入口

每个专题都支持从代码入口独立启动，不需要加载其他专题：

| 入口 | 独立专题 |
|---|---|
| `heatlab-web --topic ideal-gas` | 热力学 / 理想气体浏览器实验台 |
| `heatlab-web --topic brownian` | 布朗运动浏览器实验台 |
| `heatlab-web --topic maxwell` | 麦克斯韦分布浏览器实验台 |
| `heatlab-web --topic galton` | 伽尔顿板浏览器实验台 |
| `heatlab-ideal-gas`、`heatlab-brownian`、`heatlab-maxwell`、`heatlab-galton` | 对应的单专题 PySide6 桌面窗口 |

浏览器总览模式仍使用 `heatlab-web`。总览页也提供 `/topic/ideal-gas`、`/topic/brownian`、`/topic/maxwell`、`/topic/galton` 四个独立页面入口，便于课堂按实验组分发链接。

桌面版与 Web 版共享同一套模型层与 `Simulation/` 风格的黑色实验台视觉：顶栏（品牌 + 种子命令托盘 + 运行簇）、按实验组拆分的专题 Tab、三栏「参数 | 场景 | 图表」、底部状态栏。桌面 Matplotlib 图例/图注在深色主题下强制浅色文字并优先常规字重中文字体，避免黑字或细体字不可读。
...
1. 从 [v0.3.1 Release](https://github.com/WALKERKILLER/heatlab/releases/tag/v0.3.1) 下载 `HeatLab-Web-win64.zip`；后续版本也会沿用同一 Release 下载方式。
...
```text
下载 [HeatLab-Web-win64.zip（v0.3.1）](https://github.com/WALKERKILLER/heatlab/releases/download/v0.3.1/HeatLab-Web-win64.zip) → 解压 → 双击 HeatLab-Web.exe
```
...
# 只启动一个专题，例如布朗运动
./.venv/bin/heatlab-web --topic brownian --host 127.0.0.1 --port 8765
```
...
热力学主图按实验模块选择最直接的关系：温度微观意义与等容显示 **P-T**，等压显示 **V-T**，等温/压强微观/第一定律显示 **P-V**；需要时仍可打开 **P-V-T 相图**（给出 `(P,V,T)` 坐标值）：

- **设定路径 (PV=nRT)**：宏观状态点的 3D 轨迹  
- **当前状态**：当前 (P, V, T) 状态点，图上与参数区均标注三元组坐标值  
- **理论过程线**：等温/等压/等容模式下叠加对应理论线；第一定律使用氮气/双原子气体近似 `γ=1.4` 的准静态绝热线，并在界面说明快速动画与准静态近似的区别
- **两室热交换**：等体积、等物质的量的两室粒子不穿过隔板；导热隔板按能量守恒交换内能，隔热隔板保持两侧初始温度

麦克斯韦图表区为双图：**速率分布 f(v)** 与 **水平速度分量 v_x 高斯分布**，均含理论曲线与蒙特卡洛样本直方图。

界面为 `Simulation/` 风格的黑色扁平实验工作台 + lab-console 顶栏（brand plate、命令托盘、运行簇）。可用性：键盘 Tab、label、`aria-live` 状态栏、跳过链接、`prefers-reduced-motion`、内置 `HeatLab CJK` 字体。
...
| `POST /api/live/ideal-gas/set` · `/step` | 理想气体七组模块；`experiment_mode` 支持 `temperature-micro` / `pressure-micro` / `isothermal` / `isochoric` / `isobaric` / `heat-exchange` / `first-law`；旧 `process_mode` 仍兼容 |
...
- **理想气体**：视觉粒子忽略相互作用；T 控速度尺度，P/T 经状态方程决定 V；显示速度系数 1.5（仅影响观感，不改变物理状态）。透明粒子箱是**相对显示体积**：长度按 T/P 缩放并限制在 `0.45–1.60`，高度/深度固定为显示单位，不能理解为真实容器的三维绝对尺寸；物理体积以 `V=nRT/P` 读数为准。演示粒子数 `N_demo` 与物质的量 `n` 分开。
- **两室热交换**：两室体积和物质的量相等，采用氮气双原子近似 `C_v=5R/2`；导热隔板以固定热导率的课堂近似计算 `Q̇=K(T₁−T₂)`，每步显式守恒两室总内能并防止温度越过平衡点；这不是完整的非平衡传热 PDE。
- **热力学第一定律**：采用气体对外做功为正的符号约定 `δQ=dU+P dV`；封闭绝热过程热量为 0、内能变化加气体对外做功为 0，理论路径满足 `PV^γ=常量` 与 `TV^(γ−1)=常量`。快速拖动只用于显示显著变化，不能解读为严格准静态过程。
- **布朗运动**：原文缺黏度等 SI 参数 → 使用无量纲显式溶剂模型；质量滑条下限 `0.05 m₀`。液体分子采用 **Ornstein-Uhlenbeck 热运动 + 硬球弹性碰撞**，花粉只在实际接触时按两体弹性碰撞交换动量；`D=theta/gamma` 仅作为连续介质参考线，不再额外施加 Langevin 随机项（借鉴 OpenMM/HOOMD 的热浴处理原则与开源硬球模型的碰撞响应）
- **麦克斯韦**：默认氮气分子质量，温度 0–100 °C；显示速度系数 1.5，粒子活跃运动。粒子在显示容器的四面内壁按镜面反射（位置始终归一化在 `[0, 1]²`），单步跨越多次边界时按穿越次数正确翻转对应速度分量
