...
**核查结论：**源码已通过 Python 字节码编译检查；自动化测试结果为 **27 passed**；四组无头渲染验证图已成功生成并人工检查。桌面与 Web 两套 UI 均已实现并经过浏览器/窗口实测；GUI 的实际启动、拖动和跨平台 DPI 表现被明确列为交付前 P0 冒烟测试，不能用数值测试替代。
...
温度由摄氏度转为开尔文；压强由 atm 转为 Pa。为了让拖动条在画面上直观改变容器体积，场景采用 **3D 透明盒体**：长度方向按 `T/P` 相对参考状态缩放（高度与深度固定为 1.0），因此显示体积严格正比于 `V=nRT/P`，实际升数仍用状态方程计算。粒子位置为三维均匀采样，速度三分量独立采样自高斯分布，粒子按实时速率着色（慢=蓝 → 快=黄），温度升高时颜色整体变暖、体积膨胀，变化一目了然。
