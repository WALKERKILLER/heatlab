...
本文档给出一套可直接继续开发、运行和验收的 Python 桌面科学计算程序。实现范围严格对应原始任务文档中的四项内容：理想气体、布朗运动、气体分子速率的麦克斯韦分布、伽尔顿板蒙特卡洛模拟。原文“专题三：综合设计”仅标注“待续”，因此本版本不擅自增加第五个专题，而是预留扩展接口。

本次交付包含：

- 完整 Python 工程源码，采用 `src/` 布局；
- PySide6 桌面 GUI，四个专题分别作为 Tab；
...
**核查结论：**源码已通过 Python 字节码编译检查；自动化测试结果为 **13 passed**；四组无头渲染验证图已成功生成并人工检查。当前执行容器没有可安装的 PySide6 wheel，因此本轮没有真正启动桌面窗口；GUI 的实际启动、拖动和跨平台 DPI 表现被明确列为交付前 P0 冒烟测试，不能用数值测试替代。
...
| H-02 | 理想气体相图 | 给出 `(P,V,T)` 坐标值 | 三维 P-V-T 轨迹和当前状态点 |
| H-03 | 理想气体参数 | 温度 0–100 °C；压强 1–2 atm | 两个范围受限滑条，实时刷新 |
...
| H-06 | 麦克斯韦分布 | 固定体积，仅温度变化；大量分子热运动变化；概率–速率曲线 | 固定方盒粒子动画、理论 PDF、蒙特卡洛直方图、三种特征速度 |
...
| 打包预留 | PyInstaller | Windows/Linux 独立包 | 作为后续 TODO，当前未生成安装包 |
...
```python
manager = RandomManager(seed=20260807)
...
温度由摄氏度转为开尔文；压强由 atm 转为 Pa。为了让拖动条在画面上直观改变容器体积，二维显示容器高度固定，宽度按照 `T/P` 相对参考状态缩放并做视觉限幅。实际升数仍严格使用状态方程计算，显示限幅不修改物理数值。

单分子二维速度分量采样：

```text
v_x, v_y ~ Normal(0, k_B T / m)
```

碰壁使用弹性反射：越过左/右或上/下边界时镜像位置，并反转对应速度分量。改变温度时保留速度方向，速度乘以 `sqrt(T_new/T_old)`；改变压力时改变容器宽度并按比例重映射横坐标。
...
2. 任意滑条变化调用 `set_conditions()`；
3. `QTimer` 周期调用 `step()`；
4. 粒子散点只更新坐标，不重复创建整个画布；
5. 状态变化追加到最多 240 个 P-V-T 历史点；
6. 指标区显示 T、P、V、理论压强和随机动能论估计。
...
| `test_ideal_gas.py` | 状态方程、体积单调性、微观压强统计一致性 |
| `test_brownian.py` | 轨迹复现、系综扩散常数收敛 |
| `test_maxwell.py` | PDF 归一化、样本均速、特征速度顺序 |
...
- [ ] 在装有 PySide6 的 Windows 11 环境执行 `python -m heatlab.app`，确认窗口可启动和正常退出。
...
- [ ] PyInstaller 构建脚本和可复现 release workflow。
...
- [x] IG-08 绘制三维 P-V-T 状态轨迹。
- [x] IG-09 微观动能论压强与宏观压强统计一致。
- [~] IG-10 拖动滑条时动画和三维图实时顺畅刷新。
...
- [~] MX-10 拖动温度时粒子动画和分布图无卡顿更新。

## 13.5 伽尔顿板

- [x] GB-01 粒子从中央起点逐排左/右下落。
...
1. 记录启动系统、Python/PySide6 版本和 DPI；
2. 固定 seed 为 `20260807`，截图四个专题默认状态；
3. 每个滑条依次拖到最小、最大、默认；
4. 检查数值标签、动画、曲线和状态栏；
5. 暂停后确认粒子停止；恢复后继续；
...
| Matplotlib 动画性能有限 | 低端设备可能低帧率 | 控制粒子上限；P2 评估 PyQtGraph |
| 当前环境没有 PySide6 | 未完成窗口运行证据 | P0 在目标桌面环境冒烟测试 |
